...
class SaverMixin():
    @property
    def save_dir(self):
        return self.config.exp.save_dir
    ...
    def get_grayscale_image_(self, img, data_range, cmap):
        ...
        ic(data_range, cmap, np.min(img), np.max(img))
    ...
    def save_mesh(self, filename, v_pos, t_pos_idx, v_tex=None, t_tex_idx=None, v_rgb=None):
        ...
        print('Mesh saved to', self.get_save_path(filename))
        return self.get_save_path(filename)
    ...
    def concat_images(self, imgs, cols=8):
        h, w = imgs[0].shape[:2]
        rows = len(imgs) // cols + int(bool(len(imgs) % cols))
        print(len(imgs), rows, cols)
